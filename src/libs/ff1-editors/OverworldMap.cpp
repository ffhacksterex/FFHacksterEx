...
BOOL COverworldMap::OnInitDialog()
{
	...
	try {
		...
		minimap.Proj2 = Proj2;minimap.Context = Minimap;
		minimap.OVmap = this;
...
void COverworldMap::ReloadGraphics()
{
	...
	if(Proj2->OK_overworldtiles)
	{
		...
		for(short imagecount = 0; imagecount < 128; imagecount += 1){
			...
			Proj2->m_overworldtiles.get().Add(&bmp,TRANSPARENTCOLOR); //TODO - add Add() to FFHImages
			mDC.SelectObject(&bmp);
...
{
	Proj2->session.drawDomainGrid = (m_drawgrid.GetCheck() != 0);
	invalidate_maps();
}
